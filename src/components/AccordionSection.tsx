...
export default function AccordionSection(props: {
  ...
  return (
    <div className={props.className}>
      <ButtonBase onClick={() => props.handleChange(props.panel)} className={cls(
        cmn.fullWidth,
        cmn.flex,
        cmn.pleft,
        cmn.bordRad
        ...
        <div className={cls(cmn.m10, cmn.flex, cmn.flexg, cmn.flexcv)}>
          {props.icon ? <div className={cls(cmn.mri10, cmn.flexcv, cmn.flex, styles.chainIconxs)} >
            {props.icon}
          </div> : null}
          <p className={cls(cmn.p, cmn.p2, cmn.p700, cmn.flexg)}>
            {props.title}
          </p>
          {props.expanded === props.panel ?
            <RemoveCircleRoundedIcon className={cls(
              cmn.mri5, styles.chainIconxs, cmn.pSec)} /> :
