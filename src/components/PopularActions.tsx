...
export default function PopularActions(props: { 
  ...
  const actions = metadata.getActions(
      props.chainsMeta,
      props.chainName
    )
      ...
      <Grid container spacing={2}> 
        {actions.map((action) => (
          ...
          <SkPaper gray className={cmn.n} key={action.text}>
            <Link to={`/ecosystem/${shortAlias}/${action.app}`}
             className={cls('br__tileLogo', 'br__tileIns', cmn.flex,)}>
            <div className={cls(cmn.flex, cmn.flexcv, cmn.m10)}>

              <Logo chainsMeta={props.chainsMeta} skaleNetwork= {props.skaleNetwork} chainName={props.chainName} appName= {action.app } />
              <div>
              <div className={cls(cmn.p3, 'shortP', cmn.pPrim, cmn.mleft10, cmn.mri10)}>{action.text}</div>
              <div className={cls( cmn.p5, cmn.pSec, cmn.mri10, cmn.mleft10,)}>{description.split('.')[0]}.</div>
              </div>
              </div>
              </Link>
          </SkPaper>
